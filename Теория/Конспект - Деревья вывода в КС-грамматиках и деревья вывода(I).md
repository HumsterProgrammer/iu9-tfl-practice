...
Тем самым последовательными заменами по правилам грамматики мы получили исходный нетерминал S,  тем самым мы показали что слово принадлежит языку, двигаясь снизу-вверх - от терминалов к нетерминалам.
```dot
digraph{
	T1,T2,T4[label=a]
	T3,T5,T6[label=b]
...
Если мы смотрим снизу-вверх, то мы могли бы перевернуть правила и получив тем самым замены:
$$\begin{matrix}
b \to B \\
aBB \to B \\
\varepsilon \to S \\
...
Но в свою очередь мы можем породить это слово и сверху-вниз, последовательно раскрывая нетерминалы по правилам грамматики. *Дерево разбора* для этого слова приведено ниже.
